...
class DocumentChunk(Base):
    ...
    embedding : Mapped[Vector | None] = mapped_column(Vector(1024), nullable=True, comment = "Векторный текст")
    chunk_index : Mapped[int] = mapped_column(Integer, comment = "Порядковый номер чанка в документе")
    metadata_json : Mapped[dict | None] = mapped_column(JSONB, comment = "Дополнительные метаданные в json", default = {}, nullable = True)
    created_at : Mapped[datetime] = mapped_column(DateTime, server_default = func.now())
